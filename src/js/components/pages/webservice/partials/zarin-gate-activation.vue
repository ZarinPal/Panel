<template lang="pug">
  modal.zarin-gate(v-on:closeModal="closeModal()")
    ...
    div(slot="content")
      ...
      div.row
        purse.purses.col-lg-12.col-md-12.col-sm-12.col-xs-12(@click.native="removeErrors('purse')" v-focus="" v-validate="{ rules: {required: true}}" name="purse" v-model="purse" v-bind:data-vv-as="$i18n.t('user.purse')" :class="{'input-danger': errors.has('purse')}" v-on:select="selectedPurse"  :placeholder="$i18n.t('easypay.selectPurse')" autofocus tabindex="1")
      ...
      div.row.body-bottom
        div.col-lg-9.col-md-12.col-sm-12.col-xs-12.ta-right
          ...
          span.persian-num.activation-price(v-else)  {{ $i18n.t('webservice.zarinGateActivePrice') }} {{priceZaringate.amount | numberFormat}} {{ $i18n.t('webservice.isToman') }}
  ...
  export default {
    ...
    methods: {
      ...
      activeZarinGate() {
        ...
        this.$store.state.http.requests['webservice.postRequestZaringate'].save(
            ussdData).then(
            () => {
              let vm = this;
      ...
      getPriceZaringate() {
        this.$store.state.http.requests['webservice.getPriceZaringate'].get().
            then(response => {
