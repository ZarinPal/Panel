  ...
  div
    div.row.trans-row(@click="showDetails")
      ...
      div.col-lg-5.col-md-5.col-sm-5
        span.total-amount.persian-num {{debt.amount | numberFormat}}
  ...
  export default {
    ...
    methods: {
      ...
      reject() {
        ...
        this.$store.state.http.requests['requestMoney.postRequestMoney'].update(
            requestData).then(
            (response) => {
      ...
      changeDebtState() {
        let vm = this;
        let debtIndex = _.findIndex(
            this.$store.state.paginator.paginator.DebtList.data,
            function(debt) {
