  ...
  export default {
    ...
    created() {
      if (this.$store.state.auth.user.birthday) {
        this.birthday = moment(this.$store.state.auth.user.birthday).
    ...
    methods: {
      ...
      postLevelUp() {
        ...
        this.$store.state.http.requests['user.postInformation'].save(userData).
            then(
                () => {
                ...
                (response) => {
                  ...
                  if (response.data.meta.error_type ==
                      'UserYouCanNotEditInfo') {
                    this.step++;
                  } else {
      ...
      postUploadDocuments() {
        ...
        _.forEach(this.documentFiles, function(file, fileKey) {
          if (file) {
            ...
            vm.$http.post('https://uploads.zarinpal.com/', formData,
              ...
              if (vm.howManyFileUploaded === _.size(vm.documentFiles)) {
                ...
                vm.$store.state.http.requests['user.postUploadDocument'].save(
                    uploadedFileNames).then(
                    (response) => {
