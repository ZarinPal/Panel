    ...
    div.inner-content
        ...
        div.row.nav-page-header
            ...
            div.col-lg-6.col-md-6.col-sm-6.col-xs-6
                p.page-title {{ $t('common.transactions') }}
                p.page-description {{ $t('transaction.description') }}
            div.col-lg-6.col-md-6.col-sm-6.col-xs-6
                div.break
                button.btn.default.pull-left {{ $t('common.returnToDashboard') }}
                ...
                div.section
                    div.box
                        div.body.search-box
                            div.row
                                span.icon-search
                                span.search-title {{ $t('transaction.search') }}
                                span.search-title {{ $t('transaction.advanceSearch') }}
        ...
        div.row.filter-row
            div.col-lg-6.col-md-6.col-sm-12.col-xs-12
                span.text {{$t('transaction.purseTransactionList')}}
            ...
            div.col-lg-6.col-md-6.col-sm-12.col-xs-12
                ul.select_item.pull-left
                    li(v-ripple @click="applyGeneralFilter('all')" v-bind:class="{ active: generalFilter == 'all' }" ) {{$t('transaction.all')}}
                    li(v-ripple @click="applyGeneralFilter('1')" v-bind:class="{ active: generalFilter == '1' }")  {{$t('transaction.deposit')}}
                    li(v-ripple @click="applyGeneralFilter('-1')" v-bind:class="{ active: generalFilter == '-1' }")  {{$t('transaction.removal')}}
                    li(v-ripple @click="applyGeneralFilter('-2')" v-bind:class="{ active: generalFilter == '-2' }")  {{$t('transaction.movingOut')}}


        div.row.transaction-fields-title
            div.col-lg-2.col-md-2.col-sm-3.hidden-xs
                span {{ $t('transaction.id') }}
            div.col-lg-3.col-md-3.col-sm-3.hidden-xs
                span {{ $t('transaction.source') }}
            div.col-lg-3.col-md-3.col-sm-3.col-xs-12.hidden-title-md.hidden-xs
                span {{ $t('transaction.destination') }}
            div.col-lg-2.col-md-3.col-sm-1.col-xs-12.hidden-xs
                span {{ $t('transaction.date') }}
            div.col-lg-1.col-md-2.col-sm-1.hidden-xs
                span {{ $t('transaction.amount') }}
                small ({{ $t('transaction.toman') }})
            div.col-lg-1.col-md-2.col-sm-1.hidden-xs
                span {{ $t('common.balance') }}
                small ({{ $t('transaction.toman') }})
