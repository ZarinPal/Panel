            ...
            span(v-else)
                div(v-if="this.$store.state.auth.user.cards")
                    form(autocomplete="on" onsubmit="event.preventDefault();")
                        ...
                        div.row
                            input.ltr-input(v-mask="{money: true}" v-bind:data-vv-as="$i18n.t('transaction.amount')" maxlength="15" :class="{'input-danger': errors.has('amount')}" type="text" v-model.lazy="amount" name="amount"  id="amount" tabindex="1" @keyup="calcPercentAmount()" :placeholder="$i18n.t('card.transferAmountTitle')")
