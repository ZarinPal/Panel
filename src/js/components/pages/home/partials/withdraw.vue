            ...
            span(v-else)
                div(v-if="this.$store.state.auth.user.cards")
                    form(autocomplete="on" onsubmit="event.preventDefault();")
                        ...
                        div.row
                            input.ltr-input(v-mask="{money: true}" v-validate="{ rules: {required: true, max: 15,min_value : 5000}}" v-bind:data-vv-as="$i18n.t('card.amount')" maxlength="15" :class="{'input-danger': errors.has('amount')}" type="text" v-model="amount" name="amount"  tabindex="2" @keyup="calcPercentAmount()" :placeholder="$i18n.t('card.transferAmountTitle')")
