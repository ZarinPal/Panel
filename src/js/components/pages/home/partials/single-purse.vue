      ...
      div.middle-xs.body.single-purse-body.ta-center
        router-link(tag="li" v-bind:to="{ name: 'transaction.index', params: {type: 'purse', id: purse.purse} }")
          div.nav-balance
            ...
            div.balance-amount(v-else) در حال محاسبه
  ...
  export default {
    ...
    methods: {
      ...
      toggleEditPurse(){
        ...
        let vm = this;
        setTimeout(function() {
          let txtPurseName = document.getElementById(
              'txtPurseName-' + vm.purse.purse);
          if (txtPurseName) {
            txtPurseName.select();
          }
        }, 50);
      ...
      send() {
        ...
        this.$store.state.http.requests['purse.postEdit'].update(
            {'purse_number': this.purse.purse}, sendContent).
            then(() => {
