                    ...
                    form(autocomplete="on" onsubmit="event.preventDefault();")
                        div.row
                            ...
                            div.col-xs.no-right-margin
                                input.ltr-input(v-mask="{money: true}" v-validate="{ rules: {required: true, max: 15}}" v-bind:data-vv-as="$i18n.t('card.transferAmountTitle')" maxlength="15" :class="{'input-danger': errors.has('amount')}" type="text" v-model="amount" name="amount" id="amount" :placeholder="$i18n.t('card.transferAmountTitle')"  tabindex="1")
